...
use pcode::mcp::{McpProtocol, protocol::{Message, ProtocolHandler}, McpError};
...
fn test_message_serialization() {
    ...
    let test_cases = vec![
        Message { id: 0, payload: vec![] },
        Message { id: 1, payload: vec![0x42] },
        Message { id: u64::MAX, payload: vec![0xFF; 100] },
        Message { id: 12345, payload: (0..255).collect() },
    ];
...
fn test_protocol_handler_edge_cases() {
    ...
    // Empty payload
    let msg = Message { id: 0, payload: vec![] };
    ...
    let msg = Message { id: 999, payload: large_payload };
    let encoded = handler.encode_message(&msg).unwrap();
    let decoded = handler.decode_message(&encoded).unwrap();
...
fn test_protocol_decode_partial_message() {
    let handler = ProtocolHandler::new();
    let msg = Message { id: 123, payload: vec![1, 2, 3, 4, 5] };
    let encoded = handler.encode_message(&msg).unwrap();
