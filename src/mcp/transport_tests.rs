...
mod transport_async_tests {
    ...
    use crate::mcp::protocol::{Message, ProtocolHandler};
    use std::io;
    ...
    impl Transport for MockTransport {
        ...
        async fn receive(&mut self) -> Result<Message, io::Error> {
            if self.receive_index < self.receive_messages.len() {
                ...
                Err(io::Error::new(io::ErrorKind::UnexpectedEof, "No more messages"))
            }
        }
    }
    ...
    async fn test_mock_transport_send() {
        ...
        let msg = Message { id: 1, payload: vec![1, 2, 3] };
    ...
    async fn test_mock_transport_receive() {
        let mut transport = MockTransport::new();
        let msg = Message { id: 2, payload: vec![4, 5, 6] };
