...
use crate::mcp::streaming::{StreamingMode, StreamingTool, StreamRequest, StreamResponse};
...
impl Tool for StreamExecTool {
    ...
    async fn execute(&self, params: serde_json::Value) -> Result<serde_json::Value, ToolError> {
        let params: StreamExecParams = serde_json::from_value(params)
...
impl StreamingTool for StreamExecTool {
    ...
    async fn process_stream(
        ...
        info!("Starting streaming execution of: {} {:?}", params.command, params.args);
        
        // Start the process
        let mut child = Command::new(&params.command)
            .args(&params.args)
            .stdin(if params.stream_input { Stdio::piped() } else { Stdio::null() })
            .stdout(if params.stream_output { Stdio::piped() } else { Stdio::null() })
            .stderr(Stdio::piped())
            .spawn()
        ...
        if params.stream_output {
            if let Some(stdout) = stdout {
                ...
                tokio::spawn(async move {
                    ...
                    // Send final message
                    let _ = output_tx.send(StreamResponse {
                        id: stream_id,
                        sequence,
                        data: vec![],
                        is_last: true,
