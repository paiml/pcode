...
impl SignedManifest {
    ...
    fn sign_content(content: &ToolManifestContent, signing_key: &SigningKey) -> Signature {
        let canonical_json = serde_json::to_vec(content)
            .expect("Failed to serialize manifest content");
...
mod hex_32 {
        ...
        D: Deserializer<'de>,
    {
        ...
        if bytes.len() != 32 {
            return Err(serde::de::Error::custom("Invalid key length: expected 32 bytes"));
        }
...
mod hex_64 {
        ...
        D: Deserializer<'de>,
    {
        ...
        if bytes.len() != 64 {
            return Err(serde::de::Error::custom("Invalid signature length: expected 64 bytes"));
        }
...
mod tests {
    ...
    fn test_manifest_verifier() {
        ...
        // Add the wrong key
        verifier.add_trusted_key(&signing_key2.verifying_key().to_bytes()).unwrap();
        let trusted = verifier.verify_trusted(&signed_manifest).unwrap();
        assert!(!trusted);
        
        // Add the correct key
        verifier.add_trusted_key(&signing_key1.verifying_key().to_bytes()).unwrap();
        let trusted = verifier.verify_trusted(&signed_manifest).unwrap();
        assert!(trusted);
    }
